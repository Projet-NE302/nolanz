...
typedef struct {
    ...
    struct sockaddr_in *clientAddress;     /* clientAddress pointeur vers une structure permettant de recuperer l'adresse IP et le port du client HTTP */
} message;
...
*  cette fonction recopie les données dans un buffer et s'assure de les envoyer quand la socket est disponible (la procédure appelante peut tout de suite libérer la mémoire)
